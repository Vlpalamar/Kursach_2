...
protected:
	...
	int ID=-1;//ID юзера в сети, функция log() возвращает это значение
...
Program::Program()
{
	...
	if (fout.is_open())
	{
		if (user_list.isEmpty())
		{
			...
			Admin.name= "Aname";// админу не нужно вводить свое имя,эта и слудущее 3 строчки что бы удобнее было хранить и вычитывать из данные из файла
	...
	if (!sect_list[itSect].Sub_list[itSub].Q_list.isEmpty())
	{
		for (size_t i = ActAnsw; i < sect_list[itSect].Sub_list[itSub].Q_list.getSize(); i++)
		{
			...
			for (size_t j = 0; j < sect_list[itSect].Sub_list[itSub].Q_list[i].rightAnswer.size(); j++)
			{
				...
				//если в правильном ответе есть буквы в верхнем регистре они на нижний меняются
			}
